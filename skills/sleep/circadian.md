# Circadian — Night Owls, Larks, and Shifted Clocks

Timing problems masquerade as insomnia. The tell: sleep is normal in length and quality when the person free-runs on their own hours; it only breaks against an imposed schedule. Restriction on a mistimed clock adds deprivation without moving the clock — identify phase first.

## Identify the Phase Problem
...
## Advance Protocol (delayed phase)

The clock advances on morning light after Tmin and evening protection. Tmin = habitual wake minus 2.5 h — compute from CURRENT wake time, not the target.

1. Fix wake time at the current natural wake (not the wish), 7 days/week.
2. Light within 30 min of waking: 30 min outdoors, or a 10,000-lux box at ~40 cm while breakfasting when pre-dawn or overcast. Outdoor daylight beats the box whenever available — even overcast sky delivers several times indoor lighting.
3. Evening: dim lights and engagement rules from 2-3 h before target bedtime; no feeds in bed (arousal beats spectrum — SKILL.md Where Experts Disagree).
4. Melatonin 0.5 mg taken 5 h before target bedtime.
5. Advance the whole block (wake, light, melatonin) 15-30 min every 2-3 days; faster outruns the ~1 h/day advance ceiling and collapses into a bad week.
6. Weekends are where advances die: one 3-h Sunday sleep-in undoes the week (light lands relative to the un-advanced Tmin). Cap wake-time drift at 1 h — same line the whole skill uses.
7. Hold the final schedule ≥ 4 weeks before calling it stable; the pull back to late is strongest the first month.

Severe cases (sleep onset past 3-4am, failing work or school) → sleep specialist; home protocols move mild-to-moderate delays.
...
- Dose = intensity × duration × timing; timing dominates. 30 min at 10,000 lux placed right beats 2 h placed wrong.
- Box specs that matter: 10,000 lux at the stated distance, UV-filtered, angled from above eye line; eyes open toward it, no staring.
- Side effects worth knowing: headache and eye strain (shorten sessions), and in anyone with bipolar history morning bright light can trigger activation — that history routes to a clinician before any light protocol.
